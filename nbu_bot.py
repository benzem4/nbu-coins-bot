...
def get_coins_data():
    ...
    try:
        ...
        coins_map = {}
        available, waiting, newly_updated = [], [], []
        seen_hrefs = set()
        ...
        for link in all_links:
            ...
            p_text = parent.get_text() if parent else ""
            
            price_match = re.search(r'(\d[\d\s]*грн)', p_text)
            price = price_match.group(1) if price_match else "Очікується"
            
            is_in_stock = "наявності" in p_text.lower() or ("грн" in price.lower() and "очікується" not in p_text.lower())
            ...
            # Сортування: якщо є ціна або дата (умовно в тексті), кидаємо в "Новинки/Оновлені"
...
async def monitor_changes():
    ...
    for title, data in current_map.items():
        ...
        if result:
            ...
            # Умова 1: З'явилася в наявності
            if old_status == "WAITING" and data['status'] == "AVAILABLE":
                await notify_all(f"🔥 **ДОСТУПНО ДЛЯ ЗАМОВЛЕННЯ!**\n\n🔹 {title}\n💰 Ціна: {data['price']}\n\nПоспішайте! 🚀")
            # Умова 2: З'явилася ціна, хоча ще очікується
            elif "Очікується" in old_price and "грн" in data['price']:
                await notify_all(f"🏷 **ОНОВЛЕНО ЦІНУ (Очікується):**\n\n🔹 {title}\n💰 Нова ціна: {data['price']}\n\nСкоро у продажу! 👀")
            ...
            await notify_all(f"✨ **НОВА ПОЗИЦІЯ В КАТАЛОЗІ!**\n\n🔹 {title}\n💰 Статус: {data['price']}")
...
async def cmd_start(message: types.Message):
    ...
    kb.adjust(1, 2)
    await message.answer("✅ Моніторинг активований!", reply_markup=kb.as_markup(resize_keyboard=True))
...
async def manual_check(message: types.Message):
    ...
    res = []
    if newly:
        res.append("🆕 **ОНОВЛЕНІ / ЦІНА З'ЯВИЛАСЬ:**\n" + "\n\n".join(newly))
    if av:
        res.append("🟢 **В НАЯВНОСТІ:**\n" + "\n\n".join(av[:10]))
    if wt:
        res.append("⏳ **ОЧІКУЮТЬСЯ:**\n" + "\n\n".join(wt[:10]))
...
async def show_stats(message: types.Message):
    ...
    cur.execute("SELECT count(*) FROM last_state")
...
async def test_send(message: types.Message):
    if message.chat.id == ADMIN_ID:
        await message.answer("🚀 Тест запущено...")
        await notify_all("🧪 Тестове сповіщення системи моніторингу!")
