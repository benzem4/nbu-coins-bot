...
# --- ПАРСИНГ (ПОВЕРНУТО ДО РОБОЧОЇ ВЕРСІЇ) ---
def get_coins_data():
    headers = {
    ...
    try:
        ...
        for link in all_links:
            ...
            # Отримуємо текст блоку з ціною та статусом
            ...
            # Визначаємо статус (як було в робочій версії)
    ...
    for title, data in current_map.items():
        ...
        if result:
            ...
            cur.execute("INSERT INTO last_state (coin_title, last_status, last_price) VALUES (%s, %s, %s)", 
                        (data['status'], data['price'], title))
            # Для нових монет розсилка при першій появі
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        "⏰ Автоматичні перевірки: **09:00** та **22:00**.\n"
        "Я повідомлю вас, як тільки статус монет зміниться!",
...
async def manual_check(message: types.Message):
    ...
    text = "\n\n⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n\n".join(res) if res else "🔍 Товарів не знайдено."
...
async def test_send(message: types.Message):
    if message.chat.id == ADMIN_ID:
        await notify_all("🧪 Тест системи сповіщень працює!")
