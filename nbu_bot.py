...
async def cmd_start(message: types.Message):
    ...
    kb.adjust(1, 2)
    await message.answer("✅ Моніторинг НБУ 2.0 (Lviv Proxy) активовано!", reply_markup=kb.as_markup(resize_keyboard=True))

@dp.message(F.text == "🌐 Всі позиції (Direct)")
async def direct_list(message: types.Message):
    if message.chat.id != ADMIN_ID: return
    m = await message.answer("📡 Львівський проксі робить запит...")
    _, newly, av, wt = get_coins_data()
    all_items = newly + av + wt
    if not all_items:
        await m.edit_text("❌ Сайт не відповів. Перевір Proxy таrequirements.txt")
    else:
        await m.delete()
        text = "✅ **РЕЗУЛЬТАТ ПАРСИНГУ:**\n\n" + "\n\n".join(all_items)
...
async def manual_check(message: types.Message):
    ...
    if data is None:
        await message.answer("⚠️ Помилка зв'язку з НБУ.")
        return
    res = []
    if newly: res.append("🆕 **ЦІНИ:**\n" + "\n\n".join(newly))
    if av: res.append("🟢 **В НАЯВНОСТІ:**\n" + "\n\n".join(av[:15]))
    if wt: res.append("⏳ **ОЧІКУЮТЬСЯ:**\n" + "\n\n".join(wt[:10]))
    await message.answer(("\n\n⎯⎯⎯⎯⎯\n\n".join(res) if res else "🔍 Каталог порожній"), parse_mode="Markdown", disable_web_page_preview=True)
...
async def main():
    ...
    # СЕРВЕР ЗАПУСКАЄТЬСЯ ПЕРШИМ ДЛЯ RENDER
    app = web.Application()
